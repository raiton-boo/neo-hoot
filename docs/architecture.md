...
同じメールアドレスで別のOAuthプロバイダーからログインした場合、新規アカウントを作らず、既存の`USER`の`oauth_provider`/`oauth_id`を今回ログインしたプロバイダーの情報に更新する（アカウント連携）。ただし1ユーザーにつき同時に持てるプロバイダーは1つのみで、直近ログインした方のプロバイダーだけが有効になる。将来、複数プロバイダーを同時に紐づけたい場合は、`oauth_identities`のような別テーブルへの分割を検討する。
