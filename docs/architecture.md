...
同じメールアドレスで別のOAuthプロバイダーからログインした場合、新規アカウントを作らず、既存の`USER`に新しい`OAUTH_IDENTITY`を追加する（アカウント連携）。`oauth_provider`/`oauth_id`は`USER`本体ではなく`OAUTH_IDENTITY`という別テーブルで管理しているため、1ユーザーが複数のプロバイダーを同時に連携でき、どのプロバイダーでログインしても同一アカウントとして扱われる。
